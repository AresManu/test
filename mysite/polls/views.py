...
def OrganizationPage(request):
    ...
    request.session['finaldict']=finaldict #storingour main dict in a session wide acces for further use.
...
def OrganizationDetail(request, orga):
    contextGH = {}
    contextGH['organame']=orga
    detaildict=request.session['finaldict']
    detaildict=detaildict[orga]

    # ROOM FOR ACTION LIKE NEW API CALL using detaildict info (dict containing all member and their repo)
    #
    #
    #

    contextGH['detaildict']=detaildict #using the finaldict stored in a requessession on the page OrganizationPage.
    #DEBUG
    contextGH['DEBUG']=detaildict

    return render(request, 'polls/OrganizationDetail.html',contextGH)
